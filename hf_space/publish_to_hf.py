...
def _set_space_runtime(api: HfApi, repo_id: str, artifacts_repo_id: str, token: str) -> None:
    ...
    api.add_space_variable(repo_id=repo_id, key="CROP_LLM_PROVIDER", value="deepseek", description="LLM provider label")
    api.add_space_variable(repo_id=repo_id, key="CROP_LLM_MODEL", value="deepseek-chat", description="LLM model name")
    api.add_space_variable(repo_id=repo_id, key="CROP_LLM_ENDPOINT", value="https://api.deepseek.com/chat/completions", description="LLM chat completions endpoint")
    api.add_space_variable(repo_id=repo_id, key="CROP_LLM_API_KEY_ENV", value="DEEPSEEK_API_KEY", description="Env var name used to resolve the DeepSeek API key")
